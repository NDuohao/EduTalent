    ...
    setState(() {
      _filteredJobs = _allJobs.where((job) {
        ...
        if (_sortByNearby && widget.user.state != null && widget.user.state!.isNotEmpty) {
          final userState = widget.user.state!.toLowerCase();
          final jobState = (job.state ?? '').toLowerCase();
          final jobAddress = job.location.toLowerCase();
          
          // Match by dedicated state field or check if state name exists in full address
          matchesState = jobState.contains(userState) || jobAddress.contains(userState);
        }
